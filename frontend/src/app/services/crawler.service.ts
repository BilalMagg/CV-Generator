...
import { CrawlRequest, CrawlResponse, CrawlHistoryItem, ApiResponse } from '@app/models/crawler.types';
...
export class CrawlerService {
  ...
  async triggerCrawl(userId: string, request: CrawlRequest): Promise<CrawlResponse> {
    const response = await this.http.post<ApiResponse<CrawlResponse>>('/api/job-offers/crawl', {
      userId,
