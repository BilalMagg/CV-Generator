...
B/app/src/application-service/Controllers/ApplicationsController.csÝ7using System.ComponentModel.DataAnnotations;
...
ParseOptions.0.json–
'/app/src/application-service/Program.csÕusing Microsoft.EntityFrameworkCore;
...
public class ApplicationStatusHistoryRepository : IApplicationStatusHistoryRepository
{
    ...
    public async Task<List<ApplicationStatusHistory>> GetByApplicationIdAsync(Guid applicationId)
        => await _db.ApplicationStatusHistory
            ...
            .ToListAsync();
}ParseOptions.0.jsonµF
?/app/src/application-service/Services/ApplicationServiceImpl.csÜEusing ApplicationService.DTOs;
using ApplicationService.Entities;
...
public class ApplicationServiceImpl : IApplicationService
{
    ...
    public async Task<ApplicationResponseDto> CreateAsync(CreateApplicationDto dto, string? userId)
    {
        ...
        if (userId != null && Guid.TryParse(userId, out var userGuid))
        {
            ...
            if (!exists)
                _logger.LogWarning("Creating application for unknown user {UserId} â€” gRPC validation skipped if user-service unavailable", userId);
        }
...
ParseOptions.0.json®
>/app/src/application-service/Services/UserGrpcClientService.csÖusing CommonProtos.User;
using Grpc.Core;
