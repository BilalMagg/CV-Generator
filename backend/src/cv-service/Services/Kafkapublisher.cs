...
public class KafkaPublisher : IKafkaPublisher, IDisposable
{
    ...
    public async Task PublishAsync<T>(T evt, string topic) where T : CVEvent
    {
        try
        {
    ...
    private static string GetKey(CVEvent evt) => evt switch
    {
        CVCreatedEvent e => e.CVId.ToString(),
        CVUpdateEvent e => e.CVId.ToString(),
        CVDeletedEvent e => e.CVId.ToString(),
