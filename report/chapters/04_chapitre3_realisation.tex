...
Le projet s'appuie sur une infrastructure hybride combinant environnement local et cloud. L'infrastructure locale est provisionnée à l'aide de Terraform avec le provider VMware Workstation, permettant de déployer des machines virtuelles de manière reproductible pour les environnements de développement et de test.

\begin{itemize}
    \item \textbf{Environnement local :} VMs VMware Workstation provisionnées via Terraform (2 CPU, 2 Go RAM par VM) pour le développement et les tests.
    \item \textbf{Environnement de production :} Déploiement sur conteneurs Docker orchestrés via Docker Compose, avec possibilité d'extension vers une infrastructure cloud.
\end{itemize}
